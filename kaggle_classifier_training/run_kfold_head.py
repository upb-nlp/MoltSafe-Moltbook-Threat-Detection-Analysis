...
def read_ids(path: Path) -> list[str]:
    if not path.is_file():
        fail(f"node-id sidecar missing: {path}")
    ids = path.read_text(encoding="utf-8").splitlines()
    if not ids:
        fail(f"node-id sidecar is empty: {path}")
    return ids
...
def evaluate_fold(
    ...
    throughput_n_nodes = len(inputs.test_texts)
    throughput_nodes_per_sec = throughput_n_nodes / test_elapsed if test_elapsed > 0 else 0.0
    ...
    metrics = {
        ...
        "throughput_seconds": test_elapsed,
        "score_scale": "p_unsafe",
        "untuned_rule": "argmax: p_unsafe > p_safe",
        "final_epoch_used": cfg.max_epochs,
        "checkpoint_role": checkpoint_role(cfg),
    }
    ...
    row.update(flatten_metrics("untuned", untuned))
    return row
...
def main() -> int:
    ...
    model_name = args.model_config_name or f"{cfg.variant}_h{cfg.hidden}"
